...
.openSparse=setClass(
    "openSparse",
    slot=c(
        file="character", fileDescription="list", data="vector",
...
#' This is description
#'
#' @param file character(1) path to hdf5 file containing ....
#'
...
openSparse<-function(file){
    ...
    .openSparse(
        file=file, fileDescription=fileData, colInd=colInd,
        colDiff=colDiff, rowNum=rowNum, colNum=colNum,
        size=c(rowInd.len, colInd.len)
    )
}
    ...
    definition = function(x, na.rm=FALSE, dims=1)
{
    ...
    repeat{
        ...
        rowInd_block=h5read(
            x@file,  "mm10/indices", start=x@colInd[i1]+1,
            count=x@colInd[i2]-x@colInd[i1]
        )
        data_block=h5read(
            x@file,  "mm10/data", start=x@colInd[i1]+1,
            count=x@colInd[i2]-x@colInd[i1]
        )
        block_sparse=openiSpase(
            data_block, rowInd_block, colInd_block, x@rowNum, x@colNum
        )
    ...
    definition = function(x, na.rm=FALSE, dims=1)
{
    ...
    repeat{
        ...
        data_block=h5read(
            ...
            count=x@colInd[i2]-x@colInd[i1])
        block_sparse=openiSpase(
            data_block, rowInd_block, colInd_block, x@rowNum, i2-i1
        )
        block_sparse=upload(block_sparse)
